...
**Prior (2026-07-14):** Proved the boilerplate is **multi-agent**: added a second neutral placeholder agent (`apps/api/src/mastra/agents/summary-agent.ts`, id `summary-agent`) registered alongside `exampleAgent` in `apps/api/src/mastra/index.ts`. Each agent auto-serves its own A2A card at `/api/.well-known/:id/agent-card.json`. Fixed `callAgent()` to forward its `agentId` into JSON-RPC `params.agentId`.

**Prior (2026-07-13):** Wired Nuxt ↔ Mastra communication through the AgentBase A2A proxy with bearer token auth. Added `apps/web/server/utils/a2a-client.ts` (`callAgent()`), `apps/web/server/api/a2a/[agentId].post.ts` (Nitro proxy route), and `apps/api/src/mastra/middleware/auth.ts` (Hono middleware enforcing `AGENT_API_TOKEN` on `/api/a2a/*` and `/api/agents/*`). Added `AGENTBASE_URL`, `AGENTBASE_TOKEN`, `AGENT_API_TOKEN` to shared env schema and `.env.example`. Added agent chat demo UI to `index.vue`. Updated `docs/INTEGRATION_AGENTBASE.md` §7 with full auth flow and wiring docs. Built and tested — all green.
...
4. Build your agents/tools under `apps/api/src/mastra/` and pages under `apps/web/app/`; extend the env schema in `packages/shared/src/env.ts`.
...
apps/api/src/mastra/index.ts        ← Mastra instance (agents, storage, logger, server:4111)
apps/api/src/mastra/agents/         ← example-agent.ts + summary-agent.ts (neutral placeholders; each auto-serves its own A2A card)
apps/api/src/mastra/tools/          ← example-tool.ts (+ .spec.ts) — neutral placeholder
...
apps/api/Dockerfile         ← multi-stage build of the Mastra .mastra/output bundle (node:24-alpine)
