...
3. Fill in `docs/PROGRESS.md` §1 (name + mission) and the README title; set **one** of `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GOOGLE_GENERATIVE_AI_API_KEY` in `.env` (the agents auto-detect which provider to use), and point `DATABASE_URL` / `REDIS_URL` / `KEYCLOAK_TOKEN_ISSUER_URI` at your remote/managed services (see [.env.example](../.env.example)).
