...
| Agent telemetry        | @opentelemetry/sdk-node (+ `exporter-trace-otlp-http`, `instrumentation-http`, `instrumentation-undici`, `api`) | ^0.2x / ^1.x    | `apps/agents/src/mastra/telemetry/otel.ts` (imported first) — W3C trace propagation across the crew's in-process hops + OTLP export when `OTEL_EXPORTER_OTLP_ENDPOINT` is set (AgentBase injects it into hosted containers). Best-effort; never crashes serving.                                                |
