...
| --------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm bootstrap`            | `node scripts/bootstrap.mjs`             | New project: prompts name, auto-picks a free consecutive port block, renames placeholder, updates deps, blank git repo on `develop`                                                                                       |
| `pnpm rename <name>`        | `node scripts/rename-project.mjs <name>` | Replace the `precast` placeholder with your project name only                                                                                                                                                             |
| `pnpm set-ports`            | `node scripts/set-ports.mjs`             | Set the stack's ports (`--mastra=45000 --web=45001 --keycloak=45002`, each optional; or `--auto` to pick a fresh free consecutive block in 40000–49100) across `.env`, `.env.example`, config, pnpm scripts, Docker, docs |
